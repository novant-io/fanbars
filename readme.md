...
// render using partials
partials := ["header": Fanbars.compile(header), "footer": Fanbars.compile(footer)]
s := Fanbars.compile(template).renderStr(map, partials)
```
...
Partials use the `{{> var}}` syntax to inject content from another template. The
partial is rendered at runtime. Partials must be compiled ahead of time and passed to
the `render()` call.
